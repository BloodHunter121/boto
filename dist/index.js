...
async function sendMessageToTelegram(message, token, context) {
  ...
  console.log("Сообщение будет отпрвлено частями");
  const limit = 4e3;
...
async function requestCompletionsFromChatGPT(message, history) {
  ...
  if (resp.error?.message) {
    throw new Error(`OpenAI API Ошибка
> ${resp.error.message}
...
function mergeConfig(config, key, value) {
  switch (typeof config[key]) {
    ...
    case "object":
      ...
      throw new Error("Неподдерживаемый элемент конфигурации или ошибка данных");
    default:
      throw new Error("Неподдерживаемый элемент конфигурации или ошибка данных");
  }
}
...
var commandHandlers = {
  "/help": {
    help: "Получить справку о команде",
    scopes: ["all_private_chats", "all_chat_administrators"],
    fn: commandGetHelp
  },
  "/new": {
    help: "Начать новый диалог",
    scopes: ["all_private_chats", "all_group_chats", "all_chat_administrators"],
    fn: commandCreateNewChatContext,
    needAuth: commandAuthCheck.shareModeGroup
  },
  "/start": {
    help: "ВашID,  можете приступать к диалог",
    scopes: ["all_private_chats", "all_chat_administrators"],
    fn: commandCreateNewChatContext,
    needAuth: commandAuthCheck.default
  },
  "/img": {
    help: "Создание картинки, Полный формат команды `/img Укажите описание изображения например`/img Пляж под луной`",
    scopes: ["all_private_chats", "all_chat_administrators"],
    fn: commandGenerateImg,
    needAuth: commandAuthCheck.shareModeGroup
  },
  "/version": {
    help: "Номер текущей версии,",
    scopes: ["all_private_chats", "all_chat_administrators"],
    fn: commandFetchUpdate,
    needAuth: commandAuthCheck.default
  },
  "/setenv": {
    help: "Установить пользовательскую конфигурацию /setenv KEY=VALUE",
    scopes: [],
    fn: commandUpdateUserConfig,
    needAuth: commandAuthCheck.shareModeGroup
  },
  "/usage": {
    help: "Статистика текущего используемуего бота",
    scopes: ["all_private_chats", "all_chat_administrators"],
    fn: commandUsage,
    needAuth: commandAuthCheck.default
  },
  "/system": {
    help: "Просмотр текущей системной информации",
    scopes: ["all_private_chats", "all_chat_administrators"],
    fn: commandSystem,
    needAuth: commandAuthCheck.default
  },
...
async function commandUpdateRole(message, command, subcommand) {
  if (subcommand === "show") {
    ...
    if (size === 0) {
      return sendMessageToTelegram("Роли ещё не определены");
    }
    let showMsg = `В настоящее определены следующие роли:(${size}):
`;
...
<pre>`;
  ...
  const helpMsg = 'Ошибка формата: полный формат команды: `/role operation`\n'+
  'В настоящее время поддерживаются следующие `операции`:\n'+
  '`/role show` отображает текущие определенные роли.\n'+
  '`/role имя_роли del` удаляет роль с указанным именем.\n'+
  '`/role имя роли KEY=VALUE` устанавливает конфигурацию указанной роли.\n'+
  'В настоящее время следующие настройки:\n'+
  ' `SYSTEM_INIT_MESSAGE`: сообщение инициализации\n'+
  ' `OPENAI_API_EXTRA_PARAMS`: дополнительные параметры OpenAI API, должны быть в формате JSON';
    ...
    if (settings === "del") {
      try {
        if (USER_DEFINE.ROLE[role]) {
          ...
          return sendMessageToTelegram("Роль успешно удалена");
        }
      } catch (e) {
        return sendMessageToTelegram(`Ошибка удаления роли: \`${e.message}\``);
      }
    }
    return sendMessageToTelegram(helpMsg);
  }
  ...
  try {
    ...
    return sendMessageToTelegram("Конфигурация успешно обновлена");
  } catch (e) {
    return sendMessageToTelegram(`Ошибка формата элемента конфигурации: \`${e.message}\``);
  }
}
async function commandGenerateImg(message, command, subcommand) {
  if (subcommand === "") {
    return sendMessageToTelegram("Пожалуйста, введите описание изображения. Полный формат команды: \`/img `");
  }
  try {
    ...
    try {
      ...
      return sendMessageToTelegram(`Картинка:
${imgUrl}`);
    }
  } catch (e) {
...
async function commandCreateNewChatContext(message, command, subcommand) {
  try {
    ...
    if (command === "/new") {
      return sendMessageToTelegram("Начался новый диалог");
    } else {
      if (SHARE_CONTEXT.chatType === "private") {
        return sendMessageToTelegram(
          `Дмалог начался, ваш ID(${CURRENT_CHAT_CONTEXT.chat_id})`
        );
      } else {
        return sendMessageToTelegram(
          `Начался новый диалог,группы ID(${CURRENT_CHAT_CONTEXT.chat_id})`
        );
      }
    }
  } catch (e) {
...
async function commandUpdateUserConfig(message, command, subcommand) {
  ...
  if (kv === -1) {
    return sendMessageToTelegram(
      "Неверный формат элемента конфигурации: полный формат команды /setenv KEY=VALUE"
    );
  }
  ...
  try {
    ...
    return sendMessageToTelegram(`Ошибка формата элемнта конфигкрации: ${e.message}`);
  }
}
async function commandFetchUpdate(message, command, subcommand) {
  ...
  if (current.ts < online.ts) {
    return sendMessageToTelegram(
      `Найдена новая версия, текущая версия: ${JSON.stringify(current)}\Последнаяя версия: ${JSON.stringify(online)}`
    );
  } else {
    return sendMessageToTelegram(`В настоящее время последняя версия,текущая версия: ${JSON.stringify(current)}`);
  }
}
async function commandUsage() {
  if (!ENV.ENABLE_USAGE_STATISTICS) {
    return sendMessageToTelegram("Текущий бот не включает статистику использования");
  }
  const usage = JSON.parse(await DATABASE.get(SHARE_CONTEXT.usageKey));
  let text = "\u{1F4CA} Текущее использования бота \n\nTokens:\n";
  if (usage?.tokens) {
    const { tokens } = usage;
    const sortedChats = Object.keys(tokens.chats || {}).sort((a, b) => tokens.chats[b] - tokens.chats[a]);
    text += `- Общее использование${tokens.total || 0} tokens
- `;
...
async function commandSystem(message) {
  let msg = "Текущая системная информация выглядит следующим образом:\n";
  msg += "Модель OpenAI:" + ENV.CHAT_MODEL + "\n";
  if (ENV.DEBUG_MODE) {
    msg += "<pre>";
    msg += `USER_CONFIG: 
      ...
      try {
        if (command.needAuth) {
          ...
          if (roleList) {
            ...
            if (chatRole === null) {
              return sendMessageToTelegram("Ошибка аунтефикации");
            }
            if (!roleList.includes(chatRole)) {
              return sendMessageToTelegram(`
              Недостаточно разрешений, необходимо${roleList.join(",")},Текущий:${chatRole}`);
            }
          }
        }
      } catch (e) {
        return sendMessageToTelegram(`Ошибка аунтефикации:` + e.message);
      }
      const subcommand = message.text.substring(key.length).trim();
      try {
        return await command.fn(message, key, subcommand);
      } catch (e) {
        return sendMessageToTelegram(`Ошибка выполнения команды: ${e.message}`);
      }
    }
  }
  return null;
...
async function msgCheckEnvIsReady(message) {
  if (!ENV.API_KEY) {
    return sendMessageToTelegram("OpenAI API Key Не задан");
  }
  if (!DATABASE) {
    return sendMessageToTelegram("DATABASE Не задано");
  }
  return null;
}
async function msgFilterWhiteList(message) {
  ...
  if (SHARE_CONTEXT.chatType === "private") {
    if (!ENV.CHAT_WHITE_LIST.includes(`${CURRENT_CHAT_CONTEXT.chat_id}`)) {
      return sendMessageToTelegram(
        `У вас нет разрешения на использование этой команды. Свяжитесь с администратором, чтобы добавитьID(${CURRENT_CHAT_CONTEXT.chat_id})для добавленя в white list`
      );
    }
    return null;
  }
  if (CONST.GROUP_TYPES.includes(SHARE_CONTEXT.chatType)) {
    ...
    if (!ENV.CHAT_GROUP_WHITE_LIST.includes(`${CURRENT_CHAT_CONTEXT.chat_id}`)) {
      return sendMessageToTelegram(
        `У этой группы нет разрешения на чат. Свяжитесь с администратором, чтобы добавить группу. ID(${CURRENT_CHAT_CONTEXT.chat_id})для добавления в whitelist`
      );
    }
    return null;
  }
  return sendMessageToTelegram(
    `Этот тип еще не поддерживается(${SHARE_CONTEXT.chatType})чат`
  );
}
async function msgFilterNonTextMessage(message) {
  if (!message.text) {
    return sendMessageToTelegram("Сообщения в нетекстовых форматах в настоящее время не поддерживаются");
  }
  return null;
}
...
async function msgChatWithOpenAI(message) {
  try {
    console.log("Сррьщение с вопросом:" + message.text || "");
...
async function msgProcessByChatType(message) {
  ...
  for (const handler of handlers) {
    try {
      ...
      console.error(e);
      return sendMessageToTelegram(
        `Иметь дело с(${SHARE_CONTEXT.chatType})Ошибка в сообщении чата для`
      );
    }
  }
  return null;
